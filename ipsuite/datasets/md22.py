...
class MD22Dataset(zntrack.Node):
    ...
    datasets = {
        "Ac-Ala3-NHMe": "http://www.quantum-machine.org/gdml/repo/static/md22_Ac-Ala3-NHMe.zip",
        ...
        "AT-AT-CG-CG": "http://www.quantum-machine.org/gdml/repo/static/md22_AT-AT-CG-CG.zip",
        "buckyball-catcher": "http://www.quantum-machine.org/gdml/repo/static/md22_buckyball-catcher.zip",
    ...
    def run(self):
        ...
        if not self.dataset in self.datasets.keys():
            raise FileNotFoundError(f"Dataset {self.dataset} is not known. Key has top be in {self.datasets}")        
