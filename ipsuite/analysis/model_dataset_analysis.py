...
class ClusterdDataEvaluation(zntrack.Node):
    ...
    def run(self):
        ...
        for i, (data_cluster, label) in enumerate(zip(self.data, self.labels)):
            ...
            true_energies_per_atom = true_energies / np.array([len(atoms) for atoms in data_cluster])
            pred_energies_per_atom = pred_energies / np.array([len(atoms) for atoms in data_cluster])
        ...
        for i, (data_cluster, label) in enumerate(zip(self.data, self.labels)):
            ...
            ax2.scatter(
                true_forces,
                diff,
                color=colors[i], label=label, alpha=0.6, s=1
            )
            start_idx = end_idx
        ...
        ax3.axhline(y=overall_energy_mae, color='red', linestyle='--', alpha=0.7, 
        ...
        ax4.axhline(y=overall_forces_mae, color='red', linestyle='--', alpha=0.7,
        ...
        plt.tight_layout()
        plt.savefig(self.outs_path / "energy_error_distribution_combined.png", dpi=300, bbox_inches="tight")
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(self.outs_path / "forces_error_distribution_combined.png", dpi=300, bbox_inches="tight")
        plt.close()
