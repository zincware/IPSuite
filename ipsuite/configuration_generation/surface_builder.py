...
class AddAdsorbate(base.IPSNode):
    ...
    def _check_collision(self, new_pos: tuple[float, float], 
                        existing_positions: list[tuple[float, float]],
                        new_radius: float, existing_radii: list[float]) -> bool:
    ...
    def _find_valid_position(self, slab: ase.Atoms, radius: float,
                           existing_positions: list[tuple[float, float]],
        ...
        for attempt in range(1000):  # Limit attempts to prevent infinite loops
            ...
            if not self._check_collision((x, y), existing_positions, radius, existing_radii):
                return (x, y)
            ...
            if 'adsorbate_info' in slab.info:
                ...
                if isinstance(adsorbate_info, dict):
                    for key, value in adsorbate_info.items():
                        if hasattr(value, 'item') and value.size == 1:  # numpy scalar with single element
                            adsorbate_info[key] = value.item()
                        elif isinstance(value, np.integer):
                            adsorbate_info[key] = int(value)
                        elif isinstance(value, np.ndarray):
                            adsorbate_info[key] = value.tolist()  # Convert arrays to lists
